...
int main(void) {
	...
	*
	* The IPC_EXCL flag in conjunction with IPC_CREAT forces the
	* call to semget() to fail if the semaphore associated with
	* ipc_key exists. IPC_EXCL is useless by itself, but when combined 
	* with IPC_CREAT, it can be used as a facility to guarantee that 
	...
	* 
	* If the IPC_STAT operation is successful, the kernal
