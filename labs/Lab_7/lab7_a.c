...
int main(void) {
	...
	*
	* If the IPC_RMID control operation is successful,
	* the semaphore set is removed.
	*
	* If the IPC_RMID control operation fails, indicated by a return value
	* of -1, print error and exit program with exit code 5.
	*
	* Note: the ID of the calling process must match the creator or owner
