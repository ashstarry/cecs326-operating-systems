...
int main(int argc, char* argv[])
{
  ...
  if( (sscanf(argv[3], "%i", &num_tries)) != 1 || (sscanf(argv[2], "%i", &sleeptime)) != 1 )
  {
	printf("num_tries & sleeptime must be of type integer\n");
	exit(1);
  }
  ...
  {
	printf("sleeptime must be a positive integer less than or equal to 10\n");
	exit(1);
  }
  
  if(num_tries < 1 || num_tries > MAX_ARG)
  {
	printf("num_tries must be a positive integer less than or equal to 10\n");
	exit(1);
  }
  ...
  { 	
	//if child	
  	if((pid = fork()) == 0)
      {
         childNum = k + '0'; /* converted to option based 1 */
